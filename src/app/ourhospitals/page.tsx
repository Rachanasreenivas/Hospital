...
// ✅ CLEANUP: Importing the single, correct source of hospital data.
...
const HospitalsPage = () => {
  // ✅ CLEANUP: The manual, inconsistent list of hospitals has been removed.
  // We now use the list imported from the data file.
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        >
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            <AnimatePresence>
              {hospitals.map((hospital, index) => (
                // ✅ FIX: Link now correctly uses the 'slug' property.
                <Link href={`/hospital/${hospital.slug}`} key={hospital.id} passHref>
                  <motion.div
                    className="flex flex-col bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all border border-gray-100 group"
                    variants={fadeUp}
                    custom={index * 0.05}
                  ...
                  >
                    <div className="relative w-full h-56 overflow-hidden">
                      <Image
                        // ✅ FIX: Image now uses 'imageUrl' from the consistent data source.
